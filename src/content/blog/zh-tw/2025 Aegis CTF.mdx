---
title: 2025 神盾盃—資安競賽 Writeup
published: 2025-09-27
description: 2025 神盾盃—資安競賽 Writeup
...
# 2025 神盾盃—資安競賽 Writeup

## Web

### HiProxy

這題是使用 HAProxy 2.2.16 版本，並配置 ACL 來達成 `/guest` 和 `/admin` 流量的過濾與轉發。
在偉大的 LLM 的協助下，發現 此版本的 HAProxy 存在一個已知的 CVE-2021-40346 (HTTP request smuggling) 漏洞，
並且可以利用這個漏洞來繞過 ACL 的檢查，成功訪問到 `/admin` 頁面，取得 flag。

## Misc

### Nesting Zip

這題是給了一個 zip 檔案檔案內有一個 zip 檔案，裡面又有一個 zip 檔案，一直重複下去。
為了加快速度使用 python 的 zipfile 模組來解壓縮。
...
def extract_nested_zip(zip_path, extract_to, depth=0, max_depth=100000):
    ...
    if depth > max_depth:
        return None, f"達到最大深度 {max_depth}"
...
if __name__ == "__main__":
    ...
    print(f"完成！結果: {output_file}")
```

解壓縮完成後可以在 `nest_final.txt` 找到一串加密的文字。
文字如下：
...
先嘗試 base64 解碼，得到一串亂碼，接著嘗試使用 XOR 解密，使用 CyberChef 的 "XOR Brute Force" 功能，
嘗試所有可能的單字元 key，最後成功解出 flag。

CyberChef Receipe 如下：
...
> 這題丟 LLM 讓他自己去嘗試和迭代也解的出來。

## Reverse

### Call Back

這題是一個 Windows PE 執行檔的 CRT (C Runtime) 。  
首先，丟進 Ghidra 進行靜態分析，在點進 entry point 後，發現程式一開始會呼叫 `FUN_140001db0` 函式，如下：
...
丟給 LLM 進行分析，發現這個函式主要是負責初始化 C Runtime 環境，並呼叫 `FUN_140001530` 函式來執行主要的程式邏輯。

接著，點進 `FUN_140001530` 函式，如下：
...
void FUN_140001530(HINSTANCE param_1)
{
  ...
  hWnd = CreateWindowExW(0,local_98.lpszClassName,L"",0,0,0,0,0,(HWND)0x0,(HMENU)0x0,param_1,
                         (LPVOID)0x0);
  PostMessageW(hWnd,0x474,0,0); // 自訂訊息（WM_USER + 0x74），觸發 WndProc 中對應 case 0x474 的邏輯
...
```
這個函式主要是建立一個隱藏的視窗，並立刻觸發一個自訂的訊息 `0x474`，接著進入 Loop 等待。

所以可以判斷，處理 flag 的地方很可能在 `LAB_1400011c0` 函式（WndProc）， case 0x474 的位置，如下：
...
LAB_14000151e:
    ...
    MessageBoxW((HWND)0x0,L"Can\'t find or open file！",L"ERROR",0x10);
    PostQuitMessage(0);
    if (uStack_e8 < 0x10) goto LAB_140001341;
    pvVar11 = apvStack_100[0];
    if (0xfff < uStack_e8 + 1) {
...
MultiByteToWideChar(0, 0, (LPCSTR)&uStack_e0, -1, 
                    (LPWSTR)&uStack_a8, 0x2d); // 0x2d = 45 個寬字元的緩衝區。
```

到這邊我們已經可以看出，這個函式會嘗試打開 `flag.txt` 檔案，並讀取內容到 `uStack_e0` 開始的緩衝區中。
接著將讀取到的 flag 將讀取的 ASCII flag 轉為 UTF-16，並存放到 `uStack_a8` 開始的緩衝區中。  
...
- 配置 626 bytes 的 可讀寫執行 記憶體
- `0x40` = `PAGE_EXECUTE_READWRITE`: 允許執行 shellcode

```c
lVar9 = 4;
puVar5 = &DAT_140005040;  // <- 嵌入的加密 shellcode
pcVar6 = lpAddress;
do {
  // 每次複製 128 bytes (0x80)
...
// 再複製剩餘的 114 bytes (626 - 512)
```

- `DAT_140005040`: 存放加密/混淆的機器碼（總計 626 bytes）
- 手動展開的 memcpy（編譯器最佳化）

```c
if ((UINT)param_2 != 0x474) {  // 非觸發訊息 -> 清理
  VirtualFree(lpAddress, 0, 0x8000);
  DefWindowProcW(...);
  return;
}
...
// 執行 shellcode
iVar8 = (*lpAddress)(&uStack_e0);  // <- 呼叫動態生成的函數
                                    //   傳入 flag 字串
...
- `lpAddress(&uStack_e0)`: 將 flag 傳給動態生成的驗證函數
- return  1 → 正確，0 → 錯誤

#### Shellcode 分析

現在我們知道驗證 flag 的邏輯在 shellcode 中，因此我們用 Ghidra 將位址 `140005040` 到 `1400052B1` 
(0x140005040 + 0x271 = 0x1400052B1) 選起來強制反組譯成 Code，可以獲得下面結果：
...
這就是完整的驗證邏輯，將這段反組譯後的結果丟給 LLM 進行分析，可以得到以下的 Python 逆推程式：
    ...
    if all(32 <= b <= 126 for b in new_flag):
        ...
        for i in range(24, 32):
            if new_flag[i] < 32 or new_flag[i] > 126:
                print(f"   位置 {i}: 0x{new_flag[i]:02X}")
...
<Image src={score1} alt="小組成績" />

### 名次總表

<Image src={score2} alt="名次總表" />
